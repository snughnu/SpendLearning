...
struct CategoryUseCaseTests {
    ...
    func addCategoryCallsRepositoryWithCorrectArguments() async {
        let spy = SpyCategoryRepository()
        let sut = CategoryUseCase(repository: spy)
    ...
    func updateCategoryCallsRepositoryWithCorrectArguments() async {
        let spy = SpyCategoryRepository()
        let category = Category(name: "식비", emoji: "🍚")
        let sut = CategoryUseCase(repository: spy)

        await sut.updateCategory(category, name: "외식", emoji: "🍖")
    ...
    func deleteCategoryCallsRepositoryWithCorrectCategory() async {
        let spy = SpyCategoryRepository()
        let category = Category(name: "쇼핑", emoji: "🛍️")
        let sut = CategoryUseCase(repository: spy)

        await sut.deleteCategory(category)
    ...
    func resetToDefaultCallsRepository() async {
        let spy = SpyCategoryRepository()
        let sut = CategoryUseCase(repository: spy)

        await sut.resetToDefault()

        #expect(spy.resetCallCount == 1)
    }

    @Test("reorderCategories 호출 시 Repository의 reorderCategories가 올바른 순서로 호출된다")
    func reorderCategoriesCallsRepositoryWithCorrectOrder() async {
        let spy = SpyCategoryRepository()
